
import type { OutcodeData } from '@/types';

// Note: Type definition (OutcodeData) is now in src/types/index.ts

export const londonOutcodes: OutcodeData[] = [
  { id: 'E1', name: 'Whitechapel, Stepney, Mile End', avgPrice: 650000, priceCategory: 'medium', description: 'A vibrant and diverse area with a mix of historic and modern housing.' },
  { id: 'SW1', name: 'Westminster, Belgravia, Pimlico', avgPrice: 2500000, priceCategory: 'high', description: 'One of London\'s most prestigious and expensive areas, home to landmarks and luxury properties.' },
  { id: 'N1', name: 'Islington, Barnsbury, Canonbury', avgPrice: 900000, priceCategory: 'medium', description: 'A popular and affluent residential area known for its Georgian architecture and trendy amenities.' },
  { id: 'SE1', name: 'Waterloo, Bermondsey, Southwark', avgPrice: 750000, priceCategory: 'medium', description: 'A dynamic riverside district with significant regeneration, offering a mix of property types.' },
  { id: 'W1', name: 'Mayfair, Marylebone, Soho', avgPrice: 3000000, priceCategory: 'high', description: 'An ultra-exclusive central London area, renowned for luxury retail, dining, and prime real estate.' },
  { id: 'IG1', name: 'Ilford', avgPrice: 350000, priceCategory: 'low', description: 'An East London suburban town offering more affordable housing options with good transport links.' },
  { id: 'CR0', name: 'Croydon', avgPrice: 400000, priceCategory: 'low', description: 'A major South London hub undergoing significant development, known for its relative affordability.' },
];
